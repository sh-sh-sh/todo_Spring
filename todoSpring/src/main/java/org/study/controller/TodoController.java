...
public class TodoController {
	...
	public String list(RedirectAttributes rttr,Model model,
		...
		if(view.equals("all")&&maxpage==0) {
			...
			rttr.addFlashAttribute("msg", "등록한 할 일이 없습니다. Todo를 등록하세요!");
			return "redirect:/todo/add";
		}
		
		if(page<1||page>service.maxpage(id,view)) {
			logger.error("리스트 접근 - : 존재하지 않는 페이지 접근"+id);
			rttr.addFlashAttribute("error", "존재하지 않는 페이지에 접근해 1페이지로 이동되었습니다.");
	...
	public String view(RedirectAttributes rttr,Model model,Principal prin,
			@RequestParam(value="idx",required=true) int idx) {
		String id=prin.getName();
		if(!service.isCorrectUser(id, idx)) {
			logger.error("Todo 접근  - 올바르지 않은 접근:"+id);
			rttr.addFlashAttribute("error", "올바르지 않은 접근입니다.");
			return "redirect:/todo/list?page=1&view=all";
		}
	...
	public String done(RedirectAttributes rttr,Model model,Principal prin,
			@RequestParam(value="idx",required=true) int idx,
		...
		if(!service.setDone(idx, done)) {
			...
			rttr.addFlashAttribute("error", "상태 변경에 실패했습니다. 문의 부탁드립니다.");
			return "redirect:/todo/list?page=1&view=all";
		}
		logger.info("Todo 완료 - 성공:"+idx);
		String orgPath=(String)req.getHeader("Referer");
		rttr.addFlashAttribute("msg", "완료 상태가 변경되었습니다.");
	...
	public String edit(RedirectAttributes rttr,Model model,Principal prin,
			@RequestParam(value="idx",required=true) int idx) {
	...
	public String doEdit(RedirectAttributes rttr,Model model,@Valid TodoVO todo,BindingResult result,Principal prin) {
		...
		if(!service.isCorrectUser(id, todo.getIdx())) {
			...
			rttr.addFlashAttribute("error", "해당 할일을 수정할 권한이 없습니다.");
			return "redirect:/todo/list?page=1&view=all";
		}
		...
		if(!service.updateTodo(todo)) {
			...
			rttr.addFlashAttribute("error", "할일을 수정하지 못했습니다. 문의 부탁드립니다.");
			return "redirect:/todo/edit?idx="+todo.getIdx();
		}
		
		logger.info("Todo 수정 성공:"+todo.getIdx());
		rttr.addFlashAttribute("msg", "할일을 수정하였습니다.");
	...
	public String doAdd(RedirectAttributes rttr,Model model,TodoVO todo,Principal prin) {
		...
		rttr.addFlashAttribute("msg", "할일이 추가되었습니다.");
	...
	public String del(RedirectAttributes rttr,Model model,Principal prin,
			@RequestParam(value="idx",required=true) int idx) {
		String id=prin.getName();
		
		if(!service.isCorrectUser(id, idx)) {
			logger.error("Todo 삭제 - 권한 없음:"+id);
			rttr.addFlashAttribute("error", "해당 할일을 삭제할 권한이 없습니다.");
			return "redirect:/todo/list?page=1&view=all";
		}
		
		if(!service.deleteTodo(idx)) {
			logger.warn("Todo 삭제 - 투두를 삭제하지 못함:"+idx);
			rttr.addFlashAttribute("error", "할일을 삭제하지 못했습니다. 문의 부탁드립니다.");
			return "redirect:/todo/list?page=1&view=all";
		}
		
		logger.info("Todo 삭제 성공:"+idx);
		rttr.addFlashAttribute("msg", "할일을 삭제하였습니다.");
